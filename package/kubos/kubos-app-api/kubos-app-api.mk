#####################################################
#
# Kubos Python App API Installation
#
#####################################################
KUBOS_APP_API_VERSION = $(KUBOS_VERSION)
KUBOS_APP_API_LICENSE = Apache-2.0
KUBOS_APP_API_LICENSE_FILES = LICENSE
KUBOS_APP_API_SITE = $(KUBOS_SOURCE_DIR)/apis/app-api/python
KUBOS_APP_API_SITE_METHOD = local
KUBOS_APP_API_SETUP_TYPE = setuptools
KUBOS_APP_API_DEPENDENCIES = kubos

define KUBOS_APP_API_ADD_LEGACY_SETUP
	$(INSTALL) -D -m 0644 $(KUBOS_APP_API_PKGDIR)/setup.py \
		$(KUBOS_APP_API_DIR)/setup.py
endef
KUBOS_APP_API_POST_RSYNC_HOOKS += KUBOS_APP_API_ADD_LEGACY_SETUP

$(eval $(python-package))
